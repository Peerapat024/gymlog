      ...
      <button onClick={() => setOpen(o => !o)} style={{ width: '100%', background: D, border: 'none', padding: '14px 16px', cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ textAlign: 'left' }}>
          ...
          <div style={{ display: 'flex', gap: 12, marginTop: 4 }}>
            <span style={{ fontSize: 12, color: '#aaa', letterSpacing: '0.02em', fontWeight: 600 }}>{session.sets?.length || 0} SETS</span>
            <span style={{ fontSize: 12, color: '#aaa', letterSpacing: '0.02em', fontWeight: 600 }}>{session.duration || 0}MIN</span>
            <span style={{ fontSize: 12, color: '#aaa', letterSpacing: '0.02em', fontWeight: 600 }}>{vol.toLocaleString()} VOL</span>
            {prs > 0 && <span style={{ fontSize: 11, fontWeight: 800, color: '#000', background: A, padding: '1px 6px', borderRadius: 3 }}>{prs} PR{prs > 1 ? 'S' : ''}</span>}
          </div>
        </div>
            ...
            return (
              <div key={ex} style={{ padding: '7px 0', borderBottom: i < Object.keys(byCnt).length - 1 ? `0.5px solid #111` : 'none' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontSize: 13.5, color: '#ccc', fontWeight: 600 }}>{ex}</span>
                  <div style={{ display: 'flex', gap: 12 }}>
                    <span style={{ fontSize: 12, color: '#888' }}>{cnt} sets</span>
                    <span style={{ fontSize: 12, fontWeight: 700, color: '#999' }}>{best}KG top</span>
                  </div>
                </div>
                {sets.filter(s => s.note).map((s, ni) => (
                  <div key={ni} style={{ fontSize: 12, color: '#777', fontStyle: 'italic', marginTop: 4 }}>S{s.setNumber}: {s.note}</div>
                ))}
              </div>
            );
          })}
